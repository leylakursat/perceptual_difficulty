...
ggplot(d,aes(x=otherMod, fill=otherMod)) +
  geom_bar() +
...
```{r model2, eval=FALSE, include=FALSE}
#Mixed effects linear regression predicting logRT from fixed effects of redundant property
#Mixed effects logistic regression predicting redundant adjective use from logRT to redundant property

pd = read.csv(file="../data/pd_by_rt.csv")

pd_tomerge = pd %>%
  mutate(redundantAdj = adjQuestion) %>%
  mutate(targetName = targetname) %>%
  select(targetName,competitorName,notCompetitorName,redundantAdj,MeanRT)

tomodel = targets %>%
  mutate(redundantAdj = ifelse(trialType=="high_difficulty",clickedmaterial,clickedcolor)) %>%
  left_join(pd_tomerge, by=c("targetName","competitorName","notCompetitorName","redundantAdj")) %>%
  mutate(logRT = log(MeanRT))

# for stimuli check:
  #select(targetName,competitorName,notCompetitorName,redundantAdj) %>%
  #unique()
  
m = glmer(redundant ~ MeanRT + (1|gameid) + (1|clickedobject), data=tomodel, family="binomial")
summary(m)

m2 = lmer(logRT ~ Answer.condition*cresponder_type*ckey + (1|workerid), data=critical_cresponder)
summary(m4)
```
